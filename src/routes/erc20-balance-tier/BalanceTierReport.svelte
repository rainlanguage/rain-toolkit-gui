...
<script lang="ts">
  ...
  $: erc20address = $combineTier?.data?.combineTier?.state?.constants[0]
    ? "0x" +
      BigInt($combineTier?.data?.combineTier?.state?.constants[0]).toString(16)
    : undefined;
  ...
  const report = async () => {
    if (ethers.utils.isAddress(addressToReport)) {
      ...
      console.log(
        $combineTier?.data?.combineTier?.state?.constants[1].toString()
      );
      console.log(erc20);
