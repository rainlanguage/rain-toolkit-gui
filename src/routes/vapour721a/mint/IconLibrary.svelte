<script lang="ts">
  export let width: number = 20;
  export let height: null | number = null;
  export let color = "font-black";
  export let icon:
    | "link"
    | "alert"
    | "info"
    | "arrows"
    | "settings"
    | "double-arrow"
    | "copy"
    | "close"
    | "options"
    | "reload"
    | "view"
    | "library"
    | "forward"
    | "back"
    | "add"
    | "down-arrow"
    | "up-chevron"
    | "down-chevron"
    | "tick";
  export let inline = true;

  $: if (!height) {
    height = width;
  } else if (!width) {
    width = height;
  }
</script>

{#if icon == "link"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 17 17"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M12.8736 2.03627L8.1775 2.08605L8.1563 0.086164L16.2844 0L16.5153 7.84998L14.5162 7.90878L14.3823 3.356L8.24266 9.49563L6.82845 8.08141L12.8736 2.03627Z"
    />
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M0 4.47501C0 2.81816 1.34315 1.47501 3 1.47501H7V3.47501H3C2.44772 3.47501 2 3.92273 2 4.47501V13.475C2 14.0273 2.44772 14.475 3 14.475H12C12.5523 14.475 13 14.0273 13 13.475V8.97501H15V13.475C15 15.1319 13.6569 16.475 12 16.475H3C1.34315 16.475 0 15.1319 0 13.475V4.47501Z"
    />
  </svg>
{:else if icon == "alert"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 40 36"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M37.3205 29L22.5981 3.5C21.4434 1.5 18.5567 1.5 17.402 3.5L2.67953 29C1.52482 31 2.9682 33.5 5.2776 33.5H34.7225C37.0319 33.5 38.4752 31 37.3205 29ZM24.3302 2.5C22.4057 -0.833331 17.5944 -0.833335 15.6699 2.5L0.947475 28C-0.977026 31.3333 1.4286 35.5 5.2776 35.5H34.7225C38.5715 35.5 40.9771 31.3333 39.0526 28L24.3302 2.5Z"
    />
    <path
      d="M21.4766 22.7617H18.6641L18.2656 10.9375H21.875L21.4766 22.7617ZM20.0703 24.6133C20.6406 24.6133 21.0977 24.7812 21.4414 25.1172C21.793 25.4531 21.9688 25.8828 21.9688 26.4062C21.9688 26.9219 21.793 27.3477 21.4414 27.6836C21.0977 28.0195 20.6406 28.1875 20.0703 28.1875C19.5078 28.1875 19.0508 28.0195 18.6992 27.6836C18.3555 27.3477 18.1836 26.9219 18.1836 26.4062C18.1836 25.8906 18.3555 25.4648 18.6992 25.1289C19.0508 24.7852 19.5078 24.6133 20.0703 24.6133Z"
    />
  </svg>
{:else if icon == "info"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 38 38"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M19 34C27.2843 34 34 27.2843 34 19C34 10.7157 27.2843 4 19 4C10.7157 4 4 10.7157 4 19C4 27.2843 10.7157 34 19 34ZM19 38C29.4934 38 38 29.4934 38 19C38 8.50659 29.4934 0 19 0C8.50659 0 0 8.50659 0 19C0 29.4934 8.50659 38 19 38Z"
    />
    <path d="M16 17H22V30H16V17Z" />
    <path
      d="M22 11C22 12.6569 20.6569 14 19 14C17.3431 14 16 12.6569 16 11C16 9.34315 17.3431 8 19 8C20.6569 8 22 9.34315 22 11Z"
    />
  </svg>
{:else if icon == "arrows"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 28 35"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M14 0.5V33.2703M14 33.2703L1.5 20.7703M14 33.2703L26.5 20.7703"
      stroke="#373737"
      stroke-width="2"
    />
  </svg>
{:else if icon == "settings"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
    ><g
      ><path
        d="m13.12 24h-2.24c-.757 0-1.396-.567-1.486-1.32l-.239-1.876c-.477-.155-.937-.346-1.374-.569l-1.494 1.161c-.606.469-1.459.415-1.985-.126l-1.575-1.575c-.537-.521-.591-1.374-.122-1.979l1.161-1.495c-.224-.437-.415-.897-.569-1.374l-1.88-.239c-.75-.092-1.317-.731-1.317-1.488v-2.24c0-.757.567-1.396 1.32-1.486l1.876-.239c.155-.477.346-.937.569-1.374l-1.16-1.494c-.47-.606-.415-1.46.127-1.986l1.575-1.575c.521-.537 1.375-.59 1.979-.122l1.494 1.162c.437-.223.897-.414 1.375-.569l.239-1.88c.09-.75.729-1.317 1.486-1.317h2.24c.757 0 1.396.567 1.486 1.32l.239 1.876c.478.155.938.346 1.375.569l1.494-1.161c.607-.469 1.459-.415 1.985.127l1.575 1.575c.537.521.591 1.374.122 1.979l-1.161 1.495c.224.437.415.897.569 1.374l1.88.239c.749.091 1.316.73 1.316 1.487v2.24c0 .757-.567 1.396-1.32 1.486l-1.876.239c-.155.477-.346.937-.569 1.374l1.161 1.494c.47.606.415 1.459-.127 1.985l-1.575 1.575c-.521.537-1.375.592-1.979.122l-1.495-1.161c-.437.224-.897.415-1.374.569l-.239 1.88c-.091.75-.73 1.317-1.487 1.317zm-5.39-4.86c.083 0 .168.021.244.063.551.308 1.148.556 1.774.736.192.055.333.219.358.417l.28 2.2c.03.251.247.444.494.444h2.24c.247 0 .464-.193.493-.439l.281-2.204c.025-.198.166-.362.358-.417.626-.18 1.223-.428 1.774-.736.175-.098.393-.081.55.042l1.75 1.36c.201.156.483.143.655-.034l1.585-1.585c.181-.176.195-.458.039-.66l-1.36-1.75c-.123-.158-.14-.375-.042-.55.308-.551.556-1.148.736-1.774.055-.192.219-.333.417-.358l2.2-.28c.251-.031.444-.248.444-.495v-2.24c0-.247-.193-.464-.439-.493l-2.204-.281c-.198-.025-.362-.166-.417-.358-.18-.626-.428-1.223-.736-1.774-.098-.175-.082-.392.042-.55l1.36-1.75c.157-.202.143-.484-.033-.654l-1.585-1.585c-.175-.182-.458-.196-.66-.039l-1.75 1.36c-.159.123-.376.14-.551.042-.549-.308-1.146-.555-1.774-.736-.192-.055-.333-.219-.358-.417l-.28-2.2c-.031-.252-.248-.445-.495-.445h-2.24c-.247 0-.464.193-.493.439l-.281 2.204c-.025.198-.166.362-.358.418-.628.18-1.225.428-1.774.735-.175.099-.392.081-.551-.041l-1.75-1.36c-.202-.157-.483-.143-.654.033l-1.585 1.586c-.181.176-.195.458-.039.66l1.36 1.75c.123.158.14.375.042.55-.309.551-.556 1.148-.736 1.774-.055.192-.219.333-.417.358l-2.2.28c-.251.03-.444.247-.444.494v2.24c0 .247.193.464.439.493l2.204.281c.198.025.362.166.417.358.18.626.428 1.223.736 1.774.098.175.082.392-.042.55l-1.36 1.75c-.157.202-.143.484.033.654l1.585 1.585c.175.181.456.195.66.039l1.75-1.36c.091-.068.199-.104.308-.104z"
      /></g
    ><g
      ><path
        d="m12 17c-2.757 0-5-2.243-5-5s2.243-5 5-5 5 2.243 5 5-2.243 5-5 5zm0-9c-2.206 0-4 1.794-4 4s1.794 4 4 4 4-1.794 4-4-1.794-4-4-4z"
      /></g
    ></svg
  >
{:else if icon == "double-arrow"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 46 42"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M11 35.3734V2.99994H14V35.3734L21.9382 27.419L24.0617 29.5382L12.5 41.1234L0.938232 29.5382L3.0617 27.419L11 35.3734Z"
    />
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M35 6.62646L35 38.9999H32L32 6.62646L24.0618 14.5809L21.9383 12.4617L33.5 0.876465L45.0618 12.4617L42.9383 14.5809L35 6.62646Z"
    />
  </svg>
{:else if icon == "copy"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 54 54"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M30 6H9C7.34315 6 6 7.34315 6 9V30C6 31.6569 7.34315 33 9 33H30C31.6569 33 33 31.6569 33 30V9C33 7.34315 31.6569 6 30 6ZM9 0C4.02944 0 0 4.02944 0 9V30C0 34.9706 4.02944 39 9 39H30C34.9706 39 39 34.9706 39 30V9C39 4.02944 34.9706 0 30 0H9Z"
    />
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M43.5 15C49.299 15 54 19.701 54 25.5V42C54 48.6274 48.6274 54 42 54H25.5C19.701 54 15 49.299 15 43.5H21C21 45.9853 23.0147 48 25.5 48H42C45.3137 48 48 45.3137 48 42V25.5C48 23.0147 45.9853 21 43.5 21V15Z"
    />
  </svg>
{:else if icon == "close"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 20 20"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M10 1.5C5.30558 1.5 1.5 5.30558 1.5 10C1.5 14.6944 5.30558 18.5 10 18.5C14.6944 18.5 18.5 14.6944 18.5 10C18.5 5.30558 14.6944 1.5 10 1.5ZM0.5 10C0.5 4.75329 4.75329 0.5 10 0.5C15.2467 0.5 19.5 4.75329 19.5 10C19.5 15.2467 15.2467 19.5 10 19.5C4.75329 19.5 0.5 15.2467 0.5 10ZM9.2799 9.91346L6.27592 6.9829L6.97422 6.2671L9.99591 9.21494L13.0176 6.2671L13.7159 6.9829L10.7119 9.91346L13.7159 12.844L13.0176 13.5598L9.99591 10.612L6.97422 13.5598L6.27592 12.844L9.2799 9.91346Z"
    />
  </svg>
{:else if icon == "options"}
  <svg
    class:inline
    {width}
    {height}
    viewBox="0 0 3 13"
    class="fill-current {color}"
    xmlns="http://www.w3.org/2000/svg"
  >
    <circle cx="1.5" cy="1.5" r="1.5" />
    <circle cx="1.5" cy="6.5" r="1.5" />
    <circle cx="1.5" cy="11.5" r="1.5" />
  </svg>
{:else if icon == "reload"}
  <svg
    class:inline
    {width}
    {height}
    class={color}
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M10 17C10.5523 17 11 16.5523 11 16C11 15.4477 10.5523 15 10 15V17ZM5 16V15C4.44772 15 4 15.4477 4 16H5ZM4 21C4 21.5523 4.44772 22 5 22C5.55228 22 6 21.5523 6 21H4ZM10 15H5V17H10V15ZM4 16V21H6V16H4Z"
      class="fill-current"
    />
    <path
      d="M19.4176 14.9968C18.8569 16.3846 17.9182 17.5872 16.7081 18.468C15.498 19.3488 14.065 19.8725 12.5721 19.9795C11.0792 20.0865 9.58624 19.7726 8.26287 19.0734C6.9395 18.3742 5.83882 17.3177 5.08594 16.0241"
      class="stroke-current"
      stroke-width="2"
      stroke-linecap="round"
      stroke-linejoin="round"
    />
    <path
      d="M14 7C13.4477 7 13 7.44772 13 8C13 8.55228 13.4477 9 14 9V7ZM19 8V9C19.5523 9 20 8.55228 20 8H19ZM20 3C20 2.44772 19.5523 2 19 2C18.4477 2 18 2.44772 18 3H20ZM14 9H19V7H14V9ZM20 8V3H18V8H20Z"
      class="fill-current"
    />
    <path
      d="M4.58253 9.00315C5.14321 7.61541 6.08196 6.41278 7.29207 5.53197C8.50218 4.65116 9.93512 4.12749 11.428 4.02048C12.9209 3.91346 14.4139 4.2274 15.7373 4.92661C17.0606 5.62582 18.1613 6.68226 18.9142 7.97584"
      class="stroke-current"
      stroke-width="2"
      stroke-linecap="round"
      stroke-linejoin="round"
    />
  </svg>
{:else if icon == "view"}
  <svg
    class:inline
    {width}
    {height}
    class={color}
    viewBox="0 0 22 14"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M11 13C16.5228 13 21 7 21 7C21 7 16.5228 1 11 1C5.47715 1 1 7 1 7C1 7 5.47715 13 11 13Z"
      class="stroke-current"
      stroke-linecap="round"
      stroke-linejoin="round"
    />
    <path
      d="M11 9C12.1046 9 13 8.10457 13 7C13 5.89543 12.1046 5 11 5C9.89543 5 9 5.89543 9 7C9 8.10457 9.89543 9 11 9Z"
      class="stroke-current"
      stroke-linecap="round"
      stroke-linejoin="round"
    />
  </svg>
{:else if icon == "library"}
  <svg
    class:inline
    {width}
    {height}
    class={color}
    viewBox="0 0 20 17"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      class="fill-current {color}"
      d="M10 16L9.16795 16.5547C9.35342 16.8329 9.66565 17 10 17C10.3344 17 10.6466 16.8329 10.8321 16.5547L10 16ZM11.1094 14.3359L11.9415 14.8906L11.9415 14.8906L11.1094 14.3359ZM8.8906 14.3359L8.05855 14.8906L8.05855 14.8906L8.8906 14.3359ZM18 0H13V2H18V0ZM9 4V16H11V4H9ZM20 12V2H18V12H20ZM10.8321 16.5547L11.9415 14.8906L10.2774 13.7812L9.16795 15.4453L10.8321 16.5547ZM13.6056 14H18V12H13.6056V14ZM11.9415 14.8906C12.3124 14.3342 12.9368 14 13.6056 14V12C12.2681 12 11.0192 12.6684 10.2774 13.7812L11.9415 14.8906ZM18 12V14C19.1046 14 20 13.1046 20 12H18ZM13 0C10.7909 0 9 1.79086 9 4H11C11 2.89543 11.8954 2 13 2V0ZM18 2V2H20C20 0.89543 19.1046 0 18 0V2ZM2 2H7V0H2V2ZM2 12V2H0V12H2ZM10.8321 15.4453L9.72265 13.7812L8.05855 14.8906L9.16795 16.5547L10.8321 15.4453ZM6.39445 12H2V14H6.39445V12ZM9.72265 13.7812C8.98079 12.6684 7.73186 12 6.39445 12V14C7.06316 14 7.68762 14.3342 8.05855 14.8906L9.72265 13.7812ZM0 12C0 13.1046 0.89543 14 2 14V12H2H0ZM7 2C8.10457 2 9 2.89543 9 4H11C11 1.79086 9.20914 0 7 0V2ZM2 0C0.895431 0 0 0.89543 0 2H2V2V0Z"
    />
  </svg>
{:else if icon == "back"}
  <svg
    class:inline
    {width}
    {height}
    class={color}
    viewBox="0 0 9 16"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      class="fill-current {color}"
      d="M7.29289 15.7071C7.68342 16.0976 8.31658 16.0976 8.70711 15.7071C9.09763 15.3166 9.09763 14.6834 8.70711 14.2929L7.29289 15.7071ZM1 8L0.292893 7.29289C-0.0976311 7.68342 -0.0976311 8.31658 0.292893 8.70711L1 8ZM8.70711 1.70711C9.09763 1.31658 9.09763 0.683417 8.70711 0.292893C8.31658 -0.0976311 7.68342 -0.0976311 7.29289 0.292893L8.70711 1.70711ZM8.70711 14.2929L1.70711 7.29289L0.292893 8.70711L7.29289 15.7071L8.70711 14.2929ZM1.70711 8.70711L8.70711 1.70711L7.29289 0.292893L0.292893 7.29289L1.70711 8.70711Z"
    />
  </svg>
{:else if icon == "forward"}
  <svg
    class:inline
    {width}
    {height}
    class={color}
    viewBox="0 0 9 16"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      class="fill-current {color}"
      d="M1.70711 0.292893C1.31658 -0.0976311 0.683417 -0.0976311 0.292893 0.292893C-0.0976311 0.683417 -0.0976311 1.31658 0.292893 1.70711L1.70711 0.292893ZM8 8L8.70711 8.70711C9.09763 8.31658 9.09763 7.68342 8.70711 7.29289L8 8ZM0.292893 14.2929C-0.0976311 14.6834 -0.0976311 15.3166 0.292893 15.7071C0.683417 16.0976 1.31658 16.0976 1.70711 15.7071L0.292893 14.2929ZM0.292893 1.70711L7.29289 8.70711L8.70711 7.29289L1.70711 0.292893L0.292893 1.70711ZM7.29289 7.29289L0.292893 14.2929L1.70711 15.7071L8.70711 8.70711L7.29289 7.29289Z"
    />
  </svg>
{:else if icon == "add"}
  <svg
    class:inline
    {width}
    {height}
    class={color}
    viewBox="0 0 20 18"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      class="fill-current {color}"
      d="M18 17L18 16H18V17ZM2 17L2 18H2V17ZM1 16L2 16L2 16L1 16ZM1 3L0 3L0 3L1 3ZM19 4L20 4L20 4L19 4ZM19 16L18 16L18 16L19 16ZM18 3L18 2L18 2L18 3ZM9 13C9 13.5523 9.44772 14 10 14C10.5523 14 11 13.5523 11 13H9ZM11 7C11 6.44772 10.5523 6 10 6C9.44772 6 9 6.44772 9 7H11ZM7 9C6.44772 9 6 9.44772 6 10C6 10.5523 6.44772 11 7 11V9ZM13 11C13.5523 11 14 10.5523 14 10C14 9.44772 13.5523 9 13 9V11ZM9.2929 3.70711C9.68342 4.09763 10.3166 4.09763 10.7071 3.70711C11.0976 3.31658 11.0976 2.68342 10.7071 2.29289L9.2929 3.70711ZM8.2929 1.29289L7.58579 2L7.58579 2L8.2929 1.29289ZM1 2L2 2V2H1ZM18 16H2V18H18V16ZM2 16L2 3L0 3L7.70853e-07 16L2 16ZM18 4L18 16L20 16L20 4L18 4ZM1 4L18 4L18 2L1 2L1 4ZM20 4C20 2.89543 19.1046 2 18 2L18 4L18 4L20 4ZM2 16L2 16L7.70853e-07 16C6.74289e-07 17.1046 0.895432 18 2 18L2 16ZM18 18C19.1046 18 20 17.1046 20 16L18 16L18 16L18 18ZM11 13V7H9V13H11ZM7 11H13V9H7V11ZM10.7071 2.29289L9 0.585786L7.58579 2L9.2929 3.70711L10.7071 2.29289ZM7.58579 0H2V2H7.58579V0ZM2.62261e-06 2L0 3L2 3.00001L2 2L2.62261e-06 2ZM2 0C0.895433 0 2.6226e-06 0.895431 2.6226e-06 2H2V0ZM9 0.585786C8.62493 0.210713 8.11622 0 7.58579 0V2L7.58579 2L9 0.585786Z"
    />
  </svg>
{:else if icon == "down-arrow"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 22 22"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path d="M0 0 L12 10 L24 0" />
  </svg>
{:else if icon == "down-chevron"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 26 16"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M0.17157 2.71252L2.71252 0.17157L12.6716 10.1306L22.6306 0.17157L25.1716 2.71252L12.6716 15.2125L0.17157 2.71252Z"
    />
  </svg>
{:else if icon == "up-chevron"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 26 16"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M25.1716 12.5L22.6306 15.041L12.6716 5.08191L2.71252 15.041L0.171569 12.5L12.6716 2.14684e-06L25.1716 12.5Z"
    />
  </svg>
{:else if icon == "tick"}
  <svg
    class:inline
    class="fill-current {color}"
    {width}
    {height}
    viewBox="0 0 67 67"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M33.5 67C52.0015 67 67 52.0015 67 33.5C67 14.9985 52.0015 0 33.5 0C14.9985 0 0 14.9985 0 33.5C0 52.0015 14.9985 67 33.5 67ZM29.307 48.9095L54.745 24.9095L49.255 19.0905L26.562 40.5007L16.745 31.2387L11.255 37.0576L23.817 48.9095L26.562 51.4993L29.307 48.9095Z"
    />
  </svg>
{/if}

<style lang="postcss">
  .inline {
    @apply inline-block;
  }
</style>
