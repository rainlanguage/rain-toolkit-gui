...
describe('ERC20 : Rain Protocol Toolkit', () => {
    ...
    it(`ERC20 : Deploy an ERC20 token with custom input Configuration`, () => {
        ...
        cy.get('input[placeholder="Owner Address"]').clear().type('0x973EbeF3889daACBb9bB7f97AbfD4f6e20D26440')  //change input value according to your convenience
        cy.get('input[placeholder="Total Supply"]').clear().type('2000')  //change input value according to your convenience
        cy.contains('button', 'Deploy ERC20 Token').click()
        cy.confirmMetamaskTransaction()
        cy.wait(25000)
        cy.contains('New ERC20 Token deployed!')
    ...
    it(`ERC20 : Deploy an ERC20 token with custom Supply Control Configuration`, () => {
        ...
        cy.get('input[placeholder="Initial Supply"]').clear().type('1000') //change input value according to your convenience
        cy.get('input[placeholder="Fixed Supply"]').clear().type('2000') //change input value according to your convenience
        cy.contains('button', 'Deploy ERC20 Token').click()
        cy.confirmMetamaskTransaction()
        cy.wait(25000)
        cy.contains('New ERC20 Token deployed!')
    ...
    it(`ERC20 : Deploy an ERC20 token with custom Faucets Configuration`, () => {
        ...
        cy.get('input[placeholder="Number of blocks"]').clear().type('200') //change input value according to your convenience
        cy.get('input[placeholder="Number of token(units)"]').clear().type('1') //change input value according to your convenience
        cy.contains('button', 'Deploy ERC20 Token').click()
        cy.confirmMetamaskTransaction()
        cy.wait(25000)
        cy.contains('New ERC20 Token deployed!')
